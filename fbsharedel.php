...
if(isset($_POST['add'])){
if($page->id > 1) {
$page2 = mysql_fetch_object(mysql_query("SELECT * FROM `fbshare` WHERE `id`='{$id}' AND `user`='{$data->id}'"));
mysql_query("UPDATE `users` SET `coins`=`coins`+'{$page2->points}' WHERE `id`='{$page2->user}'");
mysql_query("UPDATE `fbshare` SET `points`='0' WHERE `id`='{$id}'");
mysql_query("DELETE FROM `fbshare` WHERE `id`='{$id}'");
mysql_query("DELETE FROM `fbshared` WHERE `site_id`='{$id}'");
...
<div>
<ul id="tabnav"> 
	<li class="tab1"><a href="fbshare.php">Earn Coins</a></li> 
	<li class="tab2"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </li> 
...
<h1>Delete Facebook Photo/Post</h1>
