...
$url0   = 'http://www.stumbleupon.com/stumbler/'. $_POST['url'] .'/channels/all'; 
$url00 = file_get_contents($url0);
$x1100090 = explode('subject-user-thumb', $url00);
$x11000901 = explode('src="', $x1100090[1]);
$x11000903 = explode('"', $x11000901[1]);
$x11000904 = $x11000903[0];

$mysql_num_rows = 0;
$verificare1 = mysql_query("SELECT * FROM `stumble` WHERE `stumble`='{$_POST['url']}'");
$verificare = mysql_num_rows($verificare1);

$verificare1 = mysql_query("SELECT * FROM `stumble` WHERE `user`='{$data->id}'");
...
$message = "Error: Enter Username only. Do NOT include HTTP:// please!"; $message2 = 1;
}
...
mysql_query("INSERT INTO `stumble` (user, stumble, title, cpc, target, top, photo) VALUES('{$data->id}', '{$protectie['url']}', '{$protectie['title']}', '{$protectie['cpc']}' , '{$protectie['setcountry11']}' , '{$protectie['smartstop']}' , '{$x11000904}'  ) ");

// Add my page to Liked
$site = mysql_fetch_object(mysql_query("SELECT * FROM `stumble` WHERE `stumble`='{$protectie['url']}'"));
...
<ul id="tabnav"> 
	...
	<li class="tab4"><a href="stumblepages.php">My Profile</a></li> 
</ul>
</div>
<h1>Stumble Followers - Add New Profile</h1>
<p>You can add your Stumble Profile to get Followers.</p>
<p>Choosing the best CPC is very important. The higher the CPC chosen the higher you will show up on the list and the more Points the person following your profile will recieve, however you will lose more points.</p>
...
<tr><td><label for="title">Title:</label></td><td width="20"></td><td><input type="text" name="title" id="title" size="25" maxlength="25" /></td></tr>
<tr><td><label for="url">Profile Username:</label></td><td width="20"></td><td><input type="text" name="url" id="url" size="25" maxlength="100" value="" /></td></tr>

<tr><td>&nbsp;</td></tr>

<tr><td><label for="points">Points per Follower (CPC):</label></td><td width="20"></td><td><select name="cpc" id="points"">
...
<tr><td colspan="2"></td><td>Remember to add points from balance to your Pages to get shares.</td></tr>
