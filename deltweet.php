...
if(isset($_POST['add'])){
if($page->id != "") {
$page2 = mysql_fetch_object(mysql_query("SELECT * FROM `tweet` WHERE `id`='{$id}' AND `user`='{$data->id}'"));
mysql_query("UPDATE `users` SET `coins`=`coins`+'{$page2->points}' WHERE `id`='{$page2->user}'");
mysql_query("UPDATE `tweet` SET `points`='0' WHERE `id`='{$id}'");
mysql_query("DELETE FROM `tweet` WHERE `id`='{$id}'");
mysql_query("DELETE FROM `tweeted` WHERE `site_id`='{$id}'");
...
<tr><td><label for="url">If you need more tweets in future, Do NOT Delete.</label></td><td width="20"></td><td></td></tr>
