...
$mysql_num_rows = 0;
$verificare1 = mysql_query("SELECT * FROM `ytdislike` WHERE `ytdislike`='{$videoname}'");
$verificare = mysql_num_rows($verificare1);

$verificare1 = mysql_query("SELECT * FROM `ytdislike` WHERE `user`='{$data->id}'");
...
if ($videoname != "") {
mysql_query("INSERT INTO `ytdislike` (user, ytdislike, title, cpc, target, top) VALUES('{$data->id}', '{$videoname}', '{$protectie['title']}', '{$protectie['cpc']}' , '{$protectie['setcountry11']}' , '{$protectie['smartstop']}'  ) ");
}

// Add my page to Liked
$site = mysql_fetch_object(mysql_query("SELECT * FROM `ytdislike` WHERE (`ytdislike`='{$videoname}' AND `user`='{$data->id}')"));
...
<p>You can add your YouTube Videos for Dis-Likes here.</p>
...
<tr><td><label for="points">Points per DisLike (CPC):</label></td><td width="20"></td><td><select name="cpc" id="points" >
...
<tr><td colspan="2"></td><td>(100) means Stop when video reach 100 real dis-likes on YouTube.</td></tr>
<tr><td colspan="2"></td><td>* TOTAL DisLikes, NOT LikesPlanet dislikes only.</td></tr>
