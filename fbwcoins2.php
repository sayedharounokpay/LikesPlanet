...
mysql_query("UPDATE `fbw` SET `points`=`points`-'{$protect['coins']}' WHERE `id`='{$id}'");
mysql_query("UPDATE `users` SET `coins`=`coins`+'{$protect['coins']}' WHERE `id`='{$data->id}'");
mysql_query("INSERT INTO statistics (user_id,date,coins_gained,fb_like,log,page) VALUES ({$data->id},NOW(),{$protect['coins']},2,'Points Retrieved From Personal Facebook Page: {$page->title}','fbwcoins2.php')");
