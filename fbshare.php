...
<h1>Facebook Shares - Share Photo/Post and Earn points/cash.</h1>
<p>To get free points/cash by sharing photos on Facebook, click on 'Share' button, Share on Facebook then Close window.</p>
...
<? 
  ...
  $siteliked4[] = -1;
  $siteliked2 = mysql_query("SELECT * FROM `fbshared` WHERE (`user_id`='{$data->id}') ");
  for($j=0; $siteliked = mysql_fetch_object($siteliked2); $j++)
	{ $siteliked4[] = $siteliked->site_id; }
  
  $site2 = mysql_query("SELECT * FROM `fbshare` WHERE (`active` = '0' AND `points` > `cpc`) AND (`target`='' OR `target` LIKE '%{$usertargetc}%' ) AND `id` NOT in (".implode(',', $siteliked4).") ORDER BY `cpc` DESC LIMIT 0, 12;");
...
?>
<div class="smartbox" id="<? echo $site->id;?>" style="height: 135px; width: 250px; background: #FFC9FF; border: 5px solid #FF99FF; border-radius: 45px; float: left; position: relative; padding: 6px; margin: 8px 8px 0 0;">
<center>
        <div><font color='green' size='3'> <b><? echo $site->cpc-1;?> Points = $<?echo number_format(($site->cpc-1)/$coinsdollar,5);?> </b></font></div>
	</br>
        <div><b> <a href="<? echo $site->fbshare;?>" target="_blank"> <font color='blue'><? echo $site->title;?></font> </a> </b> </div>
        </br>
        
        <div>
        <a href="javascript:void(0);" onclick="OpenLike('<? echo $site->id;?>', '<? echo $site->user;?>', '<? echo $site->fbshare;?>', '<? echo $site->cpc;?>');"  >
	<input type="submit" class="submit" value="Share" style="height: 38px;" />
	</a>
	</div>
...
function OpenLike(mysiteid, siteowner, mysite, cpc){
	...
	$.ajax({
		type: "GET",
		url: "fbsharestart.php",
		data: "sitename1="+mysiteid+ "---" + siteowner,
		success: function(msg){
			alert(msg);
			if (msg > 0){
			$("#Hint").html('<font size="4" color="blue">Share and Close oppened window.</font>');
			} else {
			$("#Hint").html('<font size="4" color="red">Photo link is Broken. Please skip it and share another.</font>');
			mywindow.close();
			}
		}
	});
...
function GetSubCount(mysiteaccid1, pageuserowner){
	...
	$.ajax({
		type: "GET",
		url: "fblikesconshare.php",
		data: "sitename1="+mysiteaccid1+ "---" + pageuserowner,
		success: function(msg){
			
			if (msg > 0){
			$("#Hint").html('<font size="4" color="green">Shared! '+msg+' Points added to your balance.</font>');
			...
			if (msg == 0){
			$("#Hint").html('<font size="4" color="red">Not Shared!</font>');
			}
		}
	});
