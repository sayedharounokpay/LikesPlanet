...
$site = mysql_query("SELECT * FROM `fbshare` WHERE (`id`='{$xdata[0]}' AND `user`='{$xdata[1]}') AND `id` NOT IN  (".implode(',', $siteliked4).")  ");
...
$notemsg = 'Ran Out: Your Facebook Website (' . $site->title . ') Ran out of points! Click here to Add.';
$pagehigh2 = 'http://likesplanet.com/fbwpages.php?high=' . $site->id;
$notemsg2 = 'Your Facebook Website (' . $site->fbshare . ') Ran out of points! <br /><br /> Click on link below to add more points and get more Shares.';
...
mysql_query("INSERT INTO `notebyemail` (username, title, msg, link, email) VALUES('{$userowner->login}', 'LikesPlanet.com - Your Facebook Website Ran Out of Points!', '{$notemsg2}', '{$pagehigh2}', '{$userowner->email}' )");
}}
...
mysql_query("UPDATE `fbshare` SET `likes`=`likes`+'1', `lastreallikes`='{$likesnumnum}', `points`=`points`-'{$site->cpc}' WHERE `id`='{$site->id}'");
...
mysql_query("UPDATE `fbshare` SET `active`='1' WHERE `id`='{$site->id}'");
}
}
...
mysql_query("UPDATE `stat` SET `stat`=`stat`+1 WHERE (`id`='5' or `id`='15')");
} else { echo '-1'; }
