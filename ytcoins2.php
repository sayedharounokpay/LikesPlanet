...
$id = $get['id'];
$page = mysql_fetch_object(mysql_query("SELECT * FROM `youtube` WHERE `id`='{$id}' AND `user`='{$data->id}'"));
...
mysql_query("UPDATE `youtube` SET `points`=`points`-'{$protect['coins']}' WHERE `id`='{$id}'");
...
<tr><td><label for="url">Video Title</label></td><td width="20"></td><td><? echo $page->youtube;?></td></tr>
