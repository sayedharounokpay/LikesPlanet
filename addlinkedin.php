...
$mysql_num_rows = 0;
$verificare1 = mysql_query("SELECT * FROM `linkedin` WHERE `linkedin`='{$_POST['url']}'");
$verificare = mysql_num_rows($verificare1);

$verificare1 = mysql_query("SELECT * FROM `linkedin` WHERE `user`='{$data->id}'");
$verificareC = mysql_num_rows($verificare1);

$verificare1 = mysql_query("SELECT * FROM `linkedin` WHERE `id`='{$str}'");
$verificareB = mysql_num_rows($verificare1);

if($verificare > 0) {$message = "Error: Website Added by another user!</br>If you are Page Owner, please Contact us to add."; $message2 = 1;}
...
else if(!preg_match("/http/i", $_POST['url'])){$message = "Error: HTTP:// is Not included in your URL! Please try again."; $message2 = 1;}
...
else if($_POST['addpoints'] < 30) {$message = "You should add 30 Points at least for your new website !"; $message2 = 1;}
...
mysql_query("INSERT INTO `linkedin` (user, linkedin, title, cpc, target, id) VALUES('{$data->id}', '{$protectie['url']}', '{$protectie['title']}', '{$protectie['cpc']}' , '{$protectie['setcountry11']}', '{$str}'   )");

// Add my page to Liked
$site = mysql_fetch_object(mysql_query("SELECT * FROM `linkedin` WHERE `linkedin`='{$protectie['url']}'"));
...
<h1>Linkedin Shares - Add New Website</h1>
<p>You can add your websites to get Shares on Linked.</p>
<p>Choosing the best CPC is very important. The higher the CPC chosen the higher you will show up on the list and the more Credits the person sharing your sites will recieve, however you will lose more points.</p>
<p><font color='red'>Do NOT use Symbols or Strange language in your site title, or it will be closed and stop receiving shares.</font></p>
...
<tr><td width="180"><label for="title">Title:</label></td><td width="20"></td><td><input type="text" name="title" id="title" size="40" maxlength="25" /></td></tr>
...
<tr><td><label for="aaa"></label></td><td width="20"></td><td>When site have no points it will have No shares, Even if you have points in balance.</td></tr>
...
<center>
<p>Target Shares :  <select name="target" id="target">
	<? 
...
<p>Leave target box Empty to get shares from ALL World.</p>
</center>
