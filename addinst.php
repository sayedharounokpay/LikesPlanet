...
$mysql_num_rows = 0;
$verificare1 = mysql_query("SELECT * FROM `inst` WHERE `inst`='{$_POST['url']}'");
$verificare = mysql_num_rows($verificare1);
 
$verificare1 = mysql_query("SELECT * FROM `inst` WHERE `user`='{$data->id}'");
$verificareC = mysql_num_rows($verificare1);

if($verificare > 0) {
$message = "Error: Profile Added by another user!</br>If you are Profile Owner, please Contact us to add."; $message2 = 1;
}
...
$x1103 = explode('og:image" content="', $mystring0);
...
mysql_query("INSERT INTO `inst` (user, inst, title, cpc, target, top, photo, lastreallikes) VALUES('{$data->id}', '{$protectie['url']}', '{$protectie['title']}', '{$protectie['cpc']}' , '{$protectie['setcountry11']}' , '{$protectie['smartstop']}', '{$profileimg}', '{$likesnum}'  ) ");

// Add my page to Liked
$site = mysql_fetch_object(mysql_query("SELECT * FROM `inst` WHERE `inst`='{$protectie['url']}'"));
...
mysql_query("UPDATE `inst` SET `points`=`points`+'{$_POST['addpoints']}' WHERE `id`='{$site->id}'");
mysql_query("UPDATE `users` SET `coins`=`coins`-'{$_POST['addpoints']}' WHERE `id`='{$data->id}'");
mysql_query("INSERT INTO statistics (user_id,date,coins_deducted,twitter,log,page) VALUES ({$data->id},NOW(),{$_POST['addpoints']},1,'Added Coins to Instagram Page | Page ID: {$site->id}','addg.php')");
}
...
<div>
<ul id="tabnav"> 
	<li class="tab1"><a href="inst.php">Earn Points</a></li> 
	<li class="tab2"><a href="addinst.php">Add Profile</a></li> 
	<li class="tab3"><a href="instpages.php">My Profiles</a></li> 
</ul>
</div>
<h1>Instagram Followers - Add New Profile</h1>
<p>Here, You can add your instagram to get Followers.</p>
...
<tr><td><label for="title">Profile Title:</label></td><td width="20"></td><td><input type="text" name="title" id="title" size="40" maxlength="25" /></td></tr>
<tr><td><label for="url">Profile URL:</label></td><td width="20"></td><td><input type="text" name="url" id="url" size="40" maxlength="150" value="http://instagram.com/" /></td></tr>
...
<? if ($data->pr == 0){ ?>
<p><a href="prem.php"><font color="green">Upgrade to a <b>"Premium Membership"</b> and unlock <b>15 cpc</b>, get followers even faster!</font></a><p>
<? } ?>
...
<tr><td colspan="2"></td><td>Remember to add points from balance to your Pages to get followers.</td></tr>
...
<tr><td colspan="2"></td><td>(100) means Stop when page reach 100 real followers on Instagram.</td></tr>
<tr><td colspan="2"></td><td>* TOTAL Followers, NOT LikesPlanet followers only.</td></tr>
