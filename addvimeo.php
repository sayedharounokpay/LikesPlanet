...
$splitarray = explode('o.com/', $_POST['url']);
...
else if(!preg_match("/o.com/i", $_POST['url'])){
...
$hash22 = unserialize(file_get_contents("http://vimeo.com/api/v2/video/$videoname.php"));
$videophoto = $hash22[0]['thumbnail_medium'];

if ($videoname != "") {
mysql_query("INSERT INTO `vimeo` (user, vimeo, title, cpc, target, top, photo) VALUES('{$data->id}', '{$videoname}', '{$protectie['title']}', '{$protectie['cpc']}' , '{$protectie['setcountry11']}' , '{$protectie['smartstop']}', '{$videophoto}' ) ");
}

// Add my page to Liked

$site = mysql_fetch_object(mysql_query("SELECT * FROM `vimeo` WHERE (`vimeo`='{$videoname}' AND `user`='{$data->id}')"));
mysql_query("INSERT INTO `vimeoed` (user_id, site_id) VALUES('{$data->id}','{$site->id}')");

// Add my coins to this page
if ($_POST['addpoints'] <= $data->coins  &&  $_POST['addpoints']>-1){
if ($site->id > 0) {
...
<div>
<ul id="tabnav"> 
	<li class="tab2"><a href="vimeo.php">Earn Points</a></li> 
	<li class="tab0"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </li> 
	<li class="tab3"><a href="addvimeo.php">Add Video</a></li> 
	<li class="tab4"><a href="vimeopages.php">My Videos</a></li> 
</ul>
</div>
<h1>Add Vimeo Video</h1>
<p>You can add your Vimeo Videos for Likes here.</p>
...
<tr><td><label for="url">Video URL:</label></td><td width="20"></td><td><input type="text" name="url" id="url" size="40" maxlength="43" value="http://vimeo.com/" /></td></tr>
...
<tr><td colspan="2"></td><td>(100) means Stop when video reach 100 real likes on Vimeo.</td></tr>
