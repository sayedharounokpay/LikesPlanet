...
if(isset($_POST['add'])){
if($page->id > 0) {
$page2 = mysql_fetch_object(mysql_query("SELECT * FROM `fbsub` WHERE `id`='{$id}' AND `user`='{$data->id}'"));
mysql_query("UPDATE `users` SET `coins`=`coins`+'{$page2->points}' WHERE `id`='{$page2->user}'");
mysql_query("UPDATE `fbsub` SET `points`='0' WHERE `id`='{$id}'");
mysql_query("DELETE FROM `fbsub` WHERE `id`='{$id}'");
mysql_query("DELETE FROM `subliked` WHERE `site_id`='{$id}'");
...
<div>
<ul id="tabnav"> 
	<li class="tab1"><a href="fbsubs.php">Earn Coins</a></li> 
