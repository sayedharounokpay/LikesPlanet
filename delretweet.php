...
if(isset($_POST['add'])){
if($page->id > 0) {
$page2 = mysql_fetch_object(mysql_query("SELECT * FROM `retw` WHERE `id`='{$id}' AND `user`='{$data->id}'"));
mysql_query("UPDATE `users` SET `coins`=`coins`+'{$page2->points}' WHERE `id`='{$page2->user}'");
mysql_query("UPDATE `retw` SET `points`='0' WHERE `id`='{$id}'");
mysql_query("DELETE FROM `retw` WHERE `id`='{$id}'");
mysql_query("DELETE FROM `retwed` WHERE `site_id`='{$id}'");
