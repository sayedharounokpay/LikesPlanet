...
if(isset($_POST['add'])){
if($_POST['coins'] > $page->points ) {$message = "ERROR: No Enough Coins to retract!"; $message2 = 1;}
else if($_POST['coins'] < 1) {$message = "Sorry, Minimum points to retract is 1."; $message2 = 1;}
else if(!is_numeric($_POST['coins'])){$message = "ERROR: Please enter an valid number!"; $message2 = 1;}
else{
mysql_query("UPDATE `ytdislike` SET `points`=`points`-'{$protect['coins']}' WHERE `id`='{$id}'");
...
<p>You can transfer points back from Video to your account balance.</p>
...
<tr><td><label for="url">Video Title</label></td><td width="20"></td><td><? echo $page->title;?></td></tr>
<tr><td><label for="added">Points in Video</label></td><td width="20"></td><td><? echo $page->points;?></td></tr>
<tr><td>&nbsp;</td></tr>
<tr><td><label for="add">Retract</label></td><td width="20"></td><td><input type="text" name="coins" id="add" size="3" maxlength="5" value="25" /> points. (min = 1)</td></tr>
<tr><td>&nbsp;</td></tr>
