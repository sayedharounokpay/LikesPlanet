...
if($_POST['coins'] > $data->coins){$message = "You do Not have enough points on balance!"; $message2 = 1;}
    ...
    $validify=2;
    if($result = mysql_query("SELECT id FROM ytsub WHERE id='{$page->id}'")) {
    ...
    if($validify == 1){
mysql_query("UPDATE `ytsub` SET `points`=`points`+'{$protect['coins']}' WHERE `id`='{$id}'");
...
<h1>Add Points</h1>
<p>Remember to add points to your channel each time, otherwise it will not get subscribers even if you have points in your account balance.</p>
...
<tr><td><label for="url">Channel Title</label></td><td width="20"></td><td><? echo $page->title;?></td></tr>
<tr><td><label for="added">Points in Channel</label></td><td width="20"></td><td><? echo $page->points;?></td></tr>
<tr><td>&nbsp;</td></tr>
