...
if(isset($_POST['add'])){
if($page->id != "") {
$page2 = mysql_fetch_object(mysql_query("SELECT * FROM `linkedin` WHERE `id`='{$id}' AND `user`='{$data->id}'"));
mysql_query("UPDATE `users` SET `coins`=`coins`+'{$page2->points}' WHERE `id`='{$page2->user}'");
mysql_query("UPDATE `linkedin` SET `points`='0' WHERE `id`='{$id}'");
mysql_query("DELETE FROM `linkedin` WHERE `id`='{$id}'");
mysql_query("DELETE FROM `linked` WHERE `site_id`='{$id}'");
