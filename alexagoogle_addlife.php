...
if(isset($_POST['add'])){
if($_POST['coins']*10 > $data->coins){$message = "ERROR: You don't have enough points!"; $message2 = 1;}
else if($_POST['coins'] < 1){$message = "ERROR: Please enter an valid number! minimum is 1 hour."; $message2 = 1;}
else if(!is_numeric($_POST['coins'])){$message = "ERROR: Please enter an valid number!"; $message2 = 1;}
else{
mysql_query("UPDATE `alexagoogle` SET `life`=`life`+'{$protect['coins']}' WHERE `id`='{$id}'");
mysql_query("UPDATE `users` SET `coins`=`coins`-'{$protect['coins']}'*10 WHERE `id`='{$data->id}'");
...
<ul id="tabnav"> 
	...
	<li class="tab0"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </li> 
	<li class="tab2"><a href="alexagoogle_addlife.php">Add Points/Hours</a></li>  
</ul>
</div>
<p>&nbsp;</p>
...
<tr><td><label for="add">Add</label></td><td width="20"></td><td><input type="text" name="coins" id="add" size="3" maxlength="5" value="720" /> hours. (1 hour costs 10 points)</td></tr>
<tr><td>&nbsp;</td></tr>
