...
if(isset($_POST['add'])){
if($page->id > 1) {
$page2 = mysql_fetch_object(mysql_query("SELECT * FROM `ytdislike` WHERE `id`='{$id}' AND `user`='{$data->id}'"));
mysql_query("UPDATE `users` SET `coins`=`coins`+'{$page2->points}' WHERE `id`='{$page2->user}'");
mysql_query("UPDATE `ytdislike` SET `points`='0' WHERE `id`='{$id}'");
mysql_query("DELETE FROM `ytdislike` WHERE `id`='{$id}'");
mysql_query("DELETE FROM `ytdisliked` WHERE `site_id`='{$id}'");
