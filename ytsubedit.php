...
$id = $get['id'];
$page = mysql_fetch_object(mysql_query("SELECT * FROM `ytsub` WHERE `id`='{$id}' AND `user`='{$data->id}'"));
...
mysql_query("UPDATE `ytsub` SET `title`='{$protect['title']}', `active`='{$protect['active']}', `cpc`='{$protect['cpc']}', `target`='{$protect['setcountry11']}', `top`='{$protect['smartstop']}' WHERE (`id`='{$id}' AND `user`='{$data->id}') ");
$message = "Channel Edited with success!"; $message2 = 2;
}}
?>
<body id="tab4"> 
<div>
...
<h1>Edit Channel</h1>
<p>You can change CPC or Channel title.</p>
...
<p>You can set Active 'OFF' to Stop getting subscribers.</p>
...
<tr><td><label for="url">Channel:</label></td><td width="20"></td><td><? echo $page->ytlike;?></td></tr>
...
<tr><td><label for="points">Points per Subscriber (CPC):</label></td><td width="20"></td><td><select name="cpc" id="points">
...
<tr><td colspan="2"></td><td>(100) means Stop when page reach 100 real subscribers on YouTube.</td></tr>
...
<tr><td colspan="2"></td><td><input type="submit" name="edit" value="Edit Channel Setting" class="submit" style="width: 270px; height: 38px;"  /></td></tr>
</table></center>
