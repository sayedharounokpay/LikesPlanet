...
if($_POST['coins'] > $data->coins){$message = "You do Not have enough points on balance!"; $message2 = 1;}
    ...
    $validify=2;
    if($result = mysql_query("SELECT id FROM `inst` WHERE id='{$page->id}'")) {
    ...
    if($validify == 1){
mysql_query("UPDATE `inst` SET `points`=`points`+'{$protect['coins']}' WHERE `id`='{$id}'");
...
<h1>Add Points</h1>
<p>Remember to add points to your page each time, otherwise it will not get followers even if you have points in your account balance.</p>
