...
if(isset($_POST['add'])){
if($page->id > 1) {
$page2 = mysql_fetch_object(mysql_query("SELECT * FROM `instb` WHERE `id`='{$id}' AND `user`='{$data->id}'"));
mysql_query("UPDATE `users` SET `coins`=`coins`+'{$page2->points}' WHERE `id`='{$page2->user}'");
mysql_query("UPDATE `instb` SET `points`='0' WHERE `id`='{$id}'");
mysql_query("DELETE FROM `instb` WHERE `id`='{$id}'");
mysql_query("DELETE FROM `instedb` WHERE `site_id`='{$id}'");
...
<div>
<ul id="tabnav"> 
	<li class="tab1"><a href="instb.php">Earn Coins</a></li> 
	<li class="tab2"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </li> 
