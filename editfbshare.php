...
$id = $get['id'];
$page = mysql_fetch_object(mysql_query("SELECT * FROM `fbshare` WHERE `id`='{$id}' AND `user`='{$data->id}'"));
...
mysql_query("UPDATE `fbshare` SET `title`='{$protect['title']}', `active`='{$protect['active']}', `cpc`='{$protect['cpc']}', `target`='{$protect['setcountry11']}', `top`='{$protect['smartstop']}' WHERE (`id`='{$id}' AND `user`='{$data->id}') ");
...
<p>You can set Active 'OFF' to Stop getting shares.</p>
...
<tr><td><label for="url">Photo URL</label></td><td width="20"></td><td><? echo $page->fbshare;?></td></tr>
