...
<h1>Facebook Website Shares - Share Sites and Earn points/cash.</h1>
<p>To get free points/cash by sharing others sites on Facebook, click on 'Share' button, Share on Facebook then Close window.</p>
...
<? 
  ...
  $site2 = mysql_query("SELECT * FROM `fbw` WHERE (`active` = '0' AND `points` > `cpc`) AND (`target`='' OR `target` LIKE '%{$usertargetc}%' ) AND `id` NOT in (".implode(',', $siteliked4).") ORDER BY `cpc` DESC LIMIT 0, 12;");
...
<center>
        ...
        <div><b> <a href="<? echo $site->fbw;?>" target="_blank"> <font color='blue'><? echo $site->title;?></font> </a> </b> </div>
        </br>
        
        <div>
        <a href="javascript:void(0);" onclick="OpenLike('<? echo $site->id;?>', '<? echo $site->user;?>', 'https://www.facebook.com/sharer/sharer.php?u=<? echo $site->fbw;?>', '<? echo $site->cpc;?>');"  >
...
function OpenLike(mysiteid, siteowner, mysite, cpc){
	...
	$.ajax({
		type: "GET",
		url: "fbstartlikew.php",
		data: "sitename1="+mysiteid+ "---" + siteowner,
		success: function(msg){
			if (msg > 0){
			$("#Hint").html('<font size="4" color="blue">Share Link, and Close oppened window.</font>');
			} else {
			$("#Hint").html('<font size="4" color="red">Page link is Broken. Please skip it and like another.</font>');
			}
		}
	});
