...
$id = $get['id'];
$page = mysql_fetch_object(mysql_query("SELECT * FROM `twitter` WHERE `id`='{$id}' AND `user`='{$data->id}'"));

if(isset($_POST['edit'])){
if($_POST['cpc'] < 2) {$msg = "<div class=\"msg_error\">ERROR: CPC Problem!</div>";}
else if($_POST['cpc'] > 55) {$msg = "<div class=\"msg_error\">ERROR: CPC Problem!</div>";}
else {
mysql_query("UPDATE `twitter` SET `cpc`='{$protect['cpc']}', `active`='{$protect['active']}' WHERE (`id`='{$id}' AND `user`='{$data->id}')");
$msg = "<div class=\"msg_success\">Setting edited with success!</div>";
}
}
?>
<body> 
...
<tr><td><label for="points">Points to give</label></td><td width="20"></td><td><select name="cpc" id="points">
	...
	<option value="21" <?if($page->cpc == 21){?>selected<?}?>>21</option>
	<option value="26" <?if($page->cpc == 26){?>selected<?}?>>26</option>
	<? if ($data->pr >0){ ?>
