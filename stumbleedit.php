...
$id = $get['id'];
$page = mysql_fetch_object(mysql_query("SELECT * FROM `stumble` WHERE `id`='{$id}' AND `user`='{$data->id}'"));
...
mysql_query("UPDATE `stumble` SET `title`='{$protect['title']}', `active`='{$protect['active']}', `cpc`='{$protect['cpc']}', `target`='{$protect['setcountry11']}', `top`='{$protect['smartstop']}' WHERE (`id`='{$id}' AND `user`='{$data->id}') ");
...
<tr><td><label for="url">Profile: </label></td><td width="20"></td><td><? echo $page->stumble;?></td></tr>
...
<tr><td colspan="2"></td><td>(100) means Stop when profile reach 100 real followers on Stumble.</td></tr>
