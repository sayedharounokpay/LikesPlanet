...
//$siteliked4[] = $siteliked->site_id;	
 ...
 // echo "SELECT * FROM `facebook` WHERE (`active` = '0' AND `points` > `cpc`) AND (`target`='' OR `target` LIKE '%{$usertargetc}%' ) AND `id` NOT in (".implode(',', $siteliked4).") ORDER BY `cpc` DESC LIMIT $start,$end;";
  $ext = mysql_num_rows($site2);
  
  if($data->likes_quality <= -4) {$ext = 0;}
...
<center>
	...
	<input type="submit" class="submit" value="Like" />
	</a>
	</div>
...
function OpenLike(mysiteid, siteowner, mysite, cpc){
	...
	myVarTimer = setTimeout(function(){  mywindow.close()  },29000);
	setTimeout("WaitToConfirmIt();", 1000);
	}
	else {
...
function GetSubCount(mysiteaccid1, pageuserowner){
	...
	$.ajax({
		...
		success: function(msg){
			...
			alertme = alertme +1;
				if(alertme >= 3) { alert("Alert: Give REAL Likes, or You will be limited for a hour!"); }
			} else if (msg == -99){
			$("#Hint").html('<font size="4" color="red"><b>Not Liked!</b></font>');
