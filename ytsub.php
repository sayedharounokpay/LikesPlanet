...
<ul id="tabnav">  
	...
	<li class="tab3"><a href="addytsub.php">Add Profile</a></li> 
	<li class="tab4"><a href="ytsubpages.php">My Profiles</a></li> 
</ul>
</div>

<h1>YouTube Channel Subscribers.</h1>
<p>Here you can Subscribe Youtube Channels to earn Points/Money.</p>
<p>Click on 'Subscribe' button, Subscribe profile on YouTube then 'Close Oppened Window', You can Skip profiles you do not like.</p>
<p><b>Note:</b> After you Subscribe on YouTube, Wait for 4 seconds, then close.</p>
...
<? 
  ...
  $site2 = mysql_query("SELECT * FROM `ytsub` WHERE (`active` = '0' AND `points` > `cpc`) AND (`target`='' OR `target` LIKE '%{$usertargetc}%' ) AND `id` NOT in (".implode(',', $siteliked4).") ORDER BY `cpc` DESC LIMIT 0, 9;");
...
<center>
        ...
        <div>
        <a href="javascript:void(0);" onclick="OpenLike('<? echo $site->id;?>', '<? echo $site->user;?>', '<? echo $site->ytsub;?>', '<? echo $site->cpc;?>');"  >
...
function OpenLike(mysiteid, siteowner, mysite, cpc){
	...
	mywindow = window.open("http://youtube.com/user/" + mysite, "LikesPlanet");
	...
	$.ajax({
		...
		success: function(msg){
			...
			$("#Hint").html('<font size="4" color="red">Channel link is Broken. Please skip it and like another.</font>');
			mywindow.close();
			}
		}
	});
