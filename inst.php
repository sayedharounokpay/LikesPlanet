<?php
$page_title = "Free Instagram Followers - LikesPlanet.com";
$meta_description = "Free Instagram Followers - LikesPlanet.com";
$meta_keywords = "Instagram, Followers, Free, Fans, Social Media Exchanger, LikesPlanet.com";
...
<h1>Instagram Followers - Follow Profiles and Earn points/cash.</h1>
<p>To get free points/cash by following others profiles on Instagram, click on 'Follow' button, Follow on Instagram then Close window.</p>
...
<? 
  ...
  $site2 = mysql_query("SELECT * FROM `inst` WHERE (`active` = '0' AND `points` > `cpc`) AND (`target`='' OR `target` LIKE '%{$usertargetc}%' ) AND `id` NOT in (".implode(',', $siteliked4).") ORDER BY `cpc` DESC LIMIT 0, 12;");
...
<center>
        ...
        <div><b> <a href="<? echo $site->inst;?>" target="_blank"> <font color='blue'><? echo $site->title;?></font> </a> </b> </div>
        <div><img src="<? echo $site->photo; ?>" width="89px" height="89px"></div>
        </br>
        
        <div>
        <a href="javascript:void(0);" onclick="OpenLike('<? echo $site->id;?>', '<? echo $site->user;?>', '<? echo $site->inst;?>', '<? echo $site->cpc;?>');"  >
	<input type="submit" class="submit" value="Follow"  />
	</a>
	</div>
...
function OpenLike(mysiteid, siteowner, mysite, cpc){
	...
	$.ajax({
		...
		success: function(msg){
			...
			$("#Hint").html('<font size="4" color="red">Profile link is Broken. Please skip it and follow another.</font>');
			mywindow.close();
			}
		}
	});
