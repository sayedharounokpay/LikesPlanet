...
$mysql_num_rows = 0;
$verificare1 = mysql_query("SELECT * FROM `fbw` WHERE `fbw`='{$_POST['url']}'");
...
if($verificare > 0) {
$message = "Error: Page Added by another user!</br>If you are Page Owner, please Contact us to add."; $message2 = 1;
}
...
mysql_query("INSERT INTO `fbw` (user, fbw, title, cpc, target, top) VALUES('{$data->id}', '{$protectie['url']}', '{$protectie['title']}', '{$protectie['cpc']}' , '{$protectie['setcountry11']}' , '{$protectie['smartstop']}'  ) ");

// Add my page to Liked
$site = mysql_fetch_object(mysql_query("SELECT * FROM `fbw` WHERE `fbw`='{$protectie['url']}'"));
...
mysql_query("UPDATE `fbw` SET `points`=`points`+'{$_POST['addpoints']}' WHERE `id`='{$site->id}'");
mysql_query("UPDATE `users` SET `coins`=`coins`-'{$_POST['addpoints']}' WHERE `id`='{$data->id}'");
mysql_query("INSERT INTO statistics (user_id,date,coins_deducted,fb_like,log,page) VALUES ({$data->id},NOW(),{$_POST['addpoints']},1,'Added Coins to Facebook Website (Likes) Site ID: {$site->id}','acphotocoins.php')");
}
...
<h1>Facebook Shares - Website Likes</h1>
<p>Here, You can add your websites to get Shares/Likes.</p>
